...
import { canonicalDestino } from './planoContas.js';
...
function normalizeRow(input) {
  return {
    ...
    noAuto: input.noAuto ?? false, // retirada manual: matching nao realoca
  };
}
...
export function runPipeline(inputRows, opts = {}) {
  ...
  // 3.5) canonicalizacao: o Dicionário usa grafias que divergem do template
  // ("Mútuo Financeiro L/P" vs "LP", "Fornecedores Externos" vs
  // "Fornecedores"); todo destino e' vertido para o nome exato do plano
  for (const r of rows) {
    if (r.destino) r.destino = canonicalDestino(r.destino, r.grupo, r.subCategoria);
  }
